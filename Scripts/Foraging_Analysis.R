...
###Step 1: Read and prepare CSV

#Read csv with foraging data
foraging_original <- read.csv('Data/DataCollectionSheet_Smoke.CSV')
...
###Step 2: Calculate Difference and Relative Differences
...
###Step 3: Create models, run Anovas, and test fit

##relative difference of incoming ants after 30 seconds
...
100*(m_c_r_2_model_30in$R2_conditional - m_c_r_2_model_30in$R2_marginal) # very low! (2.778908%)
...
100*(m_c_r_2_model_5in$R2_conditional - m_c_r_2_model_5in$R2_marginal) # very low! (6.896307%)
...
100*(m_c_r_2_model_30out$R2_conditional - m_c_r_2_model_30out$R2_marginal) # very low! (3.018485%) 
...
p_in <- foraging_with_differences %>%
  ...
  labs(x = "Treatment", y = "Incoming Foraging Proportion Difference\n[(Post − Pre) / Pre]") +
  scale_fill_manual(
